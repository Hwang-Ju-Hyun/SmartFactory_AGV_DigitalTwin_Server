...
- 새 calibration ID `068cdfb9c783cc74`를 physical-fleet 실행 설정에 적용했다.
- calibration 검증은 5/5, RMS `1.54 mm`, 최대 오차 `2.56 mm`, 상태 `VERIFIED`다.
- calibration 자체는 검증됐지만 새 ID로 Server/Vision HELLO와 physical-fleet pose 수신은 Server 재시작 후 별도 확인해야 한다.
...
- node 승인에 사용한 최신 `MEASURED + VERIFIED` Vision heading은 AGV/node/calibration/source session/sequence/200 ms age를 완료 시점에 다시 확인해 다음 edge start heading으로 사용한다. 하나라도 맞지 않으면 기존 incoming-link nominal heading으로 fallback하며 로그에 `headingSource`와 `visionSequence`를 남긴다. ±180도 wrap을 거친 작은 heading 차이가 불필요한 제자리회전을 만들지 않는지 시험했다.
...
2. 다음 edge가 90도 꺾이는 node에서 `phase=PRE_DEPARTURE` 회전 report, 더 새로운 Vision sequence, `dispatchResult=RELEASED`, rotate 없는 forward trajectory 순서를 확인한다.
3. 실패 시 forward가 전송되지 않았는지 확인하고 `TURN_POSITION_SPIKE`, `ERROR_NOT_DECREASING`, 2회 정렬 제한, 반복/누적 회전 guard와 마지막 pose를 기록한다.
4. Unity Editor에서 planned AGV와 Vision marker가 보정 전후 의도대로 표시되는지 확인
5. 노출된 Wi-Fi 비밀번호 변경과 저장소 secret 정리
6. 전원 분배단자, fuse, switch와 기판 고정
