...
현재 실제 TestCase0 단계는 Bezier를 제외한다. `--physical-fleet`에서 AGV 1을 node 1, 동쪽 방향으로 배치하고 COMMAND-capable firmware 연결 뒤에만 자동 임무를 시작한다. map scale은 `50 mm/unit`이고 새 정사각 격자의 각 link는 4 unit, 즉 200 mm다. 각 LINE node boundary에서 정지·ARRIVED 후 필요하면 제자리 회전한다.
