...
public:
    ...
    // 우선순위 큐(Min-Heap)에서 F 점수가 가장 낮은 노드를 위로 올리기 위한 비교자
    struct CompareNode 
    {
...
public:
    ...
    std::vector<uint32_t> FindPath(uint32_t _startNodeID, uint32_t _endNodeID, const std::unordered_map<uint32_t,MapNode>& _nodes, const std::vector<MapLink>& _links);
};